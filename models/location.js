...
//   // use inner join when master-location table will work properly
//   db.any(`select locations.loc_id AS loc_id, lat, lng, user_id from locations
//           full join master_location ON locations.loc_id = master_location.loc_id;`)
//     .then((data) => {
//       res.status(200)
//         .json(data);
...
//   db.one(`select locations.loc_id AS loc_id, lat, lng, user_id from locations
//           full join master_location ON locations.loc_id = master_location.loc_id 
//           where loc_id = $1`, locId)
//     .then((data) => {
//       res.status(200)
//         .json(data);
//     })
...
//   const userId = 1; // get from req when auth will be ready
...
//   db.tx((transaction) => {
//     const createLocationQuery = transaction.none(
//       `insert into locations (loc_id, lat, lng, population, daily_bank)
//        values('${locationId}', ${locationLat}, ${locationLng}, 10, 0)`);
//     const createMasterQuery = transaction.one(
//       `insert into master_location (user_id, loc_id, loyal_popul, daily_checkin)
//        values('${userId}', ${locationId}, 1, true)`);
